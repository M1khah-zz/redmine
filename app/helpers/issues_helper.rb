...
module IssuesHelper
  ...
  def render_issue_subject_with_tree(issue)
    ...
    ancestors.each do |ancestor|
      s << '<div> Parent task: ' + content_tag('span', link_to_issue(ancestor, :project => (issue.project_id != ancestor.project_id)))
    end
    s << '<div>'
  ...
  def render_descendants_tree(issue)
    ...
    issue_list(issue.descendants.visible.preload(:status, :priority, :tracker).sort_by(&:lft)) do |child, level|
      ...
      css << " status-#{child.status_id}"
      s << content_tag('tr',
                       content_tag('td', check_box_tag("ids[]", child.id, false, :id => nil), :class => 'checkbox') +
                           content_tag('td', link_to_issue(child, :project => (issue.project_id != child.project_id)), :class => 'subject', :style => 'width: 50%') +
                           content_tag('td', content_tag('span', h(child.status)), :class => 'status') +
                           content_tag('td', link_to_user(child.assigned_to)) +
                           content_tag('td', child.priority.name) +
                           content_tag('td', progress_bar(done_ratio, :width => '80px'))+
                           content_tag('td', child.spent_hours.round(2).to_s+'/'+child.estimated_hours.round(2).to_s),
                       :class => css)
    end
    s << '</table></form>'
    s.html_safe
  ...
  def issue_remaining_hours_details(issue)
    if issue.total_estimated_hours.present?
      if issue.total_estimated_hours == issue.estimated_hours
        l_hours_short(issue.estimated_hours-issue.total_spent_hours)
      else
        est_hours=Issue.where(" (id=? OR root_id=?) AND status_id IN(1,2,4)", issue.id(), issue.id()).sum(:estimated_hours);
        spent_hours=Issue.joins(:time_entries)
                        .where(" (issues.id=? OR issues.root_id=?) AND status_id IN(1,2,4)", issue.id(), issue.id())
                        .sum("#{TimeEntry.table_name}.hours")

        rem_hours=0
        issue.descendants.each do |child_issue|
          if !child_issue.status.is_closed
            if child_issue.estimated_hours && child_issue.spent_hours
              if child_issue.estimated_hours>child_issue.spent_hours
                rem_hours=rem_hours+child_issue.estimated_hours-child_issue.spent_hours
              end
            end
          end
        ...
        s = issue.estimated_hours.present? ? l_hours_short(issue.estimated_hours) : ""
        s << " (#{l(:label_total)}: #{l_hours_short(rem_hours)})"
        s.html_safe
      end
    end
  end
