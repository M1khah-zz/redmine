...
module ApplicationHelper
  ...
  def page_header_title
    ...
    curl=request.fullpath
    if  curl=='/projects'
      return l(:label_projects)
    end
    if  curl=='/'
      return l(:label_my_page)
    end
    if curl=='/admin'
      return l(:label_administration)
    end
    if curl=='/my/account'
      return l(:label_my_account)
    end
  ...
  def progress_bar(pcts, options={})
    begin
      if !pcts.is_a? Integer and !pcts.is_a? Float
        return
      end
  ...
  def javascript_heads
    tags = javascript_include_tag('jquery-1.11.1-ui-1.11.0-ujs-3.1.3', 'application', 'jquery.formstyler.min')
