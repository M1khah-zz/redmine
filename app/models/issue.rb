...
class Issue < ActiveRecord::Base
  ...
  after_save :reschedule_following_issues, :update_nested_set_attributes,
             :update_parent_attributes, :create_journal, :close_child_issues
  ...
  def done_ratio
    if total_estimated_hours.present?
      ...
      self_and_descendants.each do |issue|
        if (issue.status.id==6 || issue.status.id==5 || issue.status.id==3)
          if (issue.estimated_hours!=nil)
            percent+=issue.estimated_hours/total_estimated_hours
          end
        end
        puts(percent)
  ...
  def validate_issue
    ...
    if @invalid_parent_issue_id.present?
      ...
      if !valid_parent_project?(@parent_issue)
        errors.add :parent_issue_id, :invalid
      elsif (@parent_issue != parent) && (all_dependent_issues.include?(@parent_issue) || @parent_issue.all_child_issues.include?(self))
        errors.add :parent_issue_id, :invalid
      elsif !new_record?
  ...
  def close_child_issues
    if status.is_closed
      children.each do |child|
        if child.status_id!=status.id 
          child.status_id=status.id
          child.not_update_parent=true
